...
public:
	...
	inline constexpr uint64_t getNextPlayerColorHashValue(Color color) const {
		return m_hashTable[static_cast<uint32_t>(color)];
	}

	inline constexpr uint64_t getCastleHashValue(bool isCastleAvailable, Color color, bool isLongCastle) const {
